...
<template>
  <div>
    ...
    <a-typography-title :level="5" class="mb-40">Тесты</a-typography-title>
      ...
      <a-col :span="24" :lg="12" :xl="8" class="mb-24" v-for="test in tests" :key="test.id">
        <a-skeleton active :loading="loading" >
          <a-card>
            ...
            <a-card-meta :title="'Курс '+ test.title" :description="test.description">
            </a-card-meta>
          </a-card>
        </a-skeleton>
      </a-col>
...
export default ({
  ...
  methods: {
    ...
    loadData(){
      this.loading = true;
      this.$axios.get('/course'+this.$route.params.course_id+'/lesson'+ +this.$route.params.lesson_id+ '/tests')
          .then(response => {
